# =========================================================
# Lightweight n8n + Python Dockerfile
# Optimized for Render Free Plan
# Instance: 0.1 CPU / 512MB RAM
# Last updated: 2026-06-29
# =========================================================

FROM node:20-bookworm-slim

ENV DEBIAN_FRONTEND=noninteractive

# ---------------------------------------------------------
# System packages
# ---------------------------------------------------------

RUN apt-get update && apt-get install -y --no-install-recommends \
    python3 \
    python3-pip \
    python3-venv \
    ffmpeg \
    curl \
    wget \
    git \
    ca-certificates \
    tini \
    && apt-get clean \
    && rm -rf /var/lib/apt/lists/*

# ---------------------------------------------------------
# Python virtual environment
# ---------------------------------------------------------

RUN python3 -m venv /opt/venv

ENV PATH="/opt/venv/bin:$PATH"

# ---------------------------------------------------------
# Upgrade pip
# ---------------------------------------------------------

RUN pip install --no-cache-dir --upgrade pip setuptools wheel

# ---------------------------------------------------------
# Lightweight Python packages
# Optimized for low RAM
# ---------------------------------------------------------

RUN pip install --no-cache-dir \
    requests \
    python-dotenv \
    yt-dlp \
    edge-tts \
    pillow \
    beautifulsoup4 \
    lxml

# ---------------------------------------------------------
# Install stable n8n version
# Node.js 20 compatible (22+ required for newer n8n)
# ---------------------------------------------------------

RUN npm install -g n8n@1.95.3

# ---------------------------------------------------------
# Create n8n user + pre-create data directory
# ---------------------------------------------------------

RUN useradd -m -s /bin/bash n8n && \
    mkdir -p /home/n8n/.n8n && \
    chown -R n8n:n8n /home/n8n /opt/venv

USER n8n

WORKDIR /home/n8n

# ---------------------------------------------------------
# Entrypoint script — lets us configure Postgres with a single
# DB_POSTGRESDB_CONNECTION_URL env var instead of 5 separate ones.
# n8n itself does not read a connection-URL var, so this script
# parses it into the DB_POSTGRESDB_* vars n8n does read, before
# handing off to n8n.
# ---------------------------------------------------------

COPY --chown=n8n:n8n entrypoint.sh /home/n8n/entrypoint.sh
RUN chmod +x /home/n8n/entrypoint.sh

# ---------------------------------------------------------
# Core settings
# ---------------------------------------------------------

ENV N8N_HOST=0.0.0.0
ENV N8N_PORT=5678

# Persistent storage — Render Disk mount path: /home/n8n/.n8n
ENV N8N_DATA_FOLDER=/home/n8n/.n8n

# ---------------------------------------------------------
# Memory optimization (512MB RAM)
# n8n main process: 256MB
# Task runner:      128MB
# OS + buffer:      128MB
# ---------------------------------------------------------

ENV NODE_OPTIONS=--max-old-space-size=280

# FIX: Removed EXECUTIONS_PROCESS — deprecated in n8n 1.95.3

# ---------------------------------------------------------
# Task Runners
# FIX (2026-08-04): Disabled. On Render's free 0.1 CPU tier the
# task broker's grant token (15s TTL) expires before the runner
# process can spawn and register, causing an infinite
# "Task runner connection attempt failed with status code 403"
# retry loop that eats CPU and can break Code nodes.
# (n8n GitHub issue #28638 — low-power hardware grant-token race.)
# Legacy in-process Code node execution is used instead, which is
# lighter and matches the existing NODE_FUNCTION_ALLOW_BUILTIN=fs
# setting below (that var only takes effect on the legacy runner).
# ---------------------------------------------------------

ENV N8N_RUNNERS_ENABLED=false

# ---------------------------------------------------------
# Execution data cleanup
# Prevents DB from growing and consuming RAM
# ---------------------------------------------------------

ENV N8N_PRUNING_ENABLED=true
ENV N8N_PRUNING_EXECUTION_DATA_MAX_AGE=24

# ---------------------------------------------------------
# Disable unnecessary features
# ---------------------------------------------------------

ENV N8N_DIAGNOSTICS_ENABLED=false
ENV N8N_VERSION_NOTIFICATIONS_ENABLED=false
ENV N8N_TEMPLATES_ENABLED=false

# Disable queue/worker mode (not needed, saves RAM)
ENV QUEUE_HEALTH_CHECK_ACTIVE=false
ENV OFFLOAD_MANUAL_EXECUTIONS_TO_WORKERS=false

# ---------------------------------------------------------
# Security
# ---------------------------------------------------------

ENV N8N_SECURE_COOKIE=false

# ---------------------------------------------------------
# Healthcheck — Render জানবে service কখন ready
# ---------------------------------------------------------

HEALTHCHECK --interval=30s --timeout=10s --start-period=40s --retries=3 \
    CMD curl -f http://localhost:5678/healthz || exit 1

# ---------------------------------------------------------
# Expose port
# ---------------------------------------------------------

EXPOSE 5678

# ---------------------------------------------------------
# Start n8n with tini (proper signal handling)
# ---------------------------------------------------------

ENTRYPOINT ["tini", "--", "/home/n8n/entrypoint.sh"]
CMD ["n8n", "start"]
